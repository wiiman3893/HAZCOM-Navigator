...
export function receiver({transport,replica,files}) {
  ...
  return {
    sync:companyId=>serial(async()=>{
      ...
      demand(metadata.status==='published' && metadata.schemaVersion===1 && metadata.companyId===companyId && metadata.company?.id===companyId && metadata.revisionId===revisionId && metadata.revisionNumber===access.currentRevisionNumber,'Revision identity mismatch');
      const raw={};
      ...
      const attachments=await transport.attachments(companyId,revisionId);
      demand(Number.isInteger(metadata.attachmentCount) && attachments.length===metadata.attachmentCount && attachments.length<=limits.attachments,'Incomplete SDS manifest');
      ...
      for(const a of attachments) {
        ...
        demand(a.published===true && a.ownerType==='chemical_product' && a.slotKey==='sds' && dataset.chemicalProducts.some(p=>p.id===a.ownerId),'Invalid SDS ownership');
        ...
        demand(a.relativePath===attachmentPath(companyId,revisionId,a),'SDS revision/path mismatch');
      }
      if(access.role!=='member') demand(await digest({rows:dataset,attachmentIds:[...ids].sort()})===metadata.payloadHash,'Revision payload hash mismatch');
      const attempt=crypto.randomUUID(), staged=[];
