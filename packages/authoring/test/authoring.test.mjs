...
const migration=migration3+migration4;
...
test('Small / Medium / Large authoring query, filter and edit measurements',async()=>{
 ...
 if(!process.env.HAZCOM_DIAGNOSTICS_NO_MEASUREMENTS)await writeFile(new URL('../../../docs/windows-authoring-measurements.json',import.meta.url),JSON.stringify({executedAt:new Date().toISOString(),node:process.version,measurements},null,2)+'\n');console.log(JSON.stringify(measurements));
});
