...
function Harness(){const [companyId,setCompany]=useState('ui-company'),[role,setRole]=useState('manager'),[entitlement,setEntitlement]=useState('paid');const open=useCallback(async()=>{
 ...
 return Object.fromEntries(['snapshot','create','update','trash','importSds','unlinkSds','readSds','importSdsBatch','splitSdsImportDraft','mergeSdsImportDraft','saveSdsImportDrafts'].map(method=>[method,(...args:any[])=>invoke(method,args.map(a=>a instanceof Uint8Array?Array.from(a):a))]));
 },[companyId,role]);
